...
class GradingMLTracker:
    """
    Patent Claim 8: ML Training Data Tracker for Grading Predictions
    Records AI predictions vs actual PSA/BGS grades to improve accuracy over time
    """

    def __init__(self, db_path: str = "E:/MTTGG/AI_LEARNING/grading_training.db"):
        self.db_path = Path(db_path)
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        self._init_database()
        print("[OK] Grading ML Tracker initialized")
    ...
    def _init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS grading_predictions (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                card_name TEXT,
                predicted_grade REAL,
                actual_grade REAL,
                centering_predicted REAL,
                corners_predicted REAL,
                edges_predicted REAL,
                surface_predicted REAL,
                confidence REAL,
    ...
    def record_prediction(self, card_name: str, prediction: Dict, image_hash: str = None):
        """Record an AI grading prediction before submission"""
        import sqlite3
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()

        cursor.execute('''
            INSERT INTO grading_predictions
            (card_name, predicted_grade, centering_predicted, corners_predicted,
             edges_predicted, surface_predicted, confidence, submission_date, image_hash)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            card_name,
            prediction.get('overall_grade'),
            prediction.get('centering_score'),
            prediction.get('corners_score'),
            prediction.get('edges_score'),
            prediction.get('surface_score'),
        ...
        print(f"[ML] Recorded prediction #{prediction_id}: {card_name} -> Grade {prediction.get('overall_grade')}")
        return prediction_id

    def record_actual_grade(self, prediction_id: int, actual_grade: float,
                           grading_company: str = "PSA", notes: str = None):
        """Record the actual grade received from PSA/BGS/etc"""
        import sqlite3
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()

        cursor.execute('''
            UPDATE grading_predictions
            SET actual_grade = ?, grading_company = ?, result_date = ?, notes = ?
            WHERE id = ?
        ''', (actual_grade, grading_company, datetime.now().isoformat(), notes, prediction_id))
        ...
        print(f"[ML] Recorded actual grade for #{prediction_id}: {actual_grade} ({grading_company})")
    ...
    def _update_accuracy_metrics(self):
        ...
        cursor.execute('''
            SELECT predicted_grade, actual_grade
            FROM grading_predictions
            WHERE actual_grade IS NOT NULL
        ''')
        ...
        exact_matches = sum(1 for p, a in predictions if abs(p - a) < 0.1)
        within_one = sum(1 for p, a in predictions if abs(p - a) <= 1.0)
        avg_deviation = sum(abs(p - a) for p, a in predictions) / total

        accuracy = (within_one / total) * 100

        cursor.execute('''
            INSERT INTO accuracy_metrics
            (timestamp, total_predictions, exact_matches, within_one, avg_deviation, accuracy_percentage)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (datetime.now().isoformat(), total, exact_matches, within_one, avg_deviation, accuracy))

        conn.commit()
        conn.close()

        print(f"[ML] Accuracy updated: {accuracy:.1f}% within 1 grade ({total} samples)")

    def get_accuracy_report(self) -> Dict:
        """Get current ML accuracy metrics"""
        import sqlite3
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()

        cursor.execute('''
            SELECT * FROM accuracy_metrics ORDER BY id DESC LIMIT 1
        ''')
        ...
        if not row:
            return {'status': 'No training data yet', 'samples': 0}

        return {
            'timestamp': row[1],
            'total_predictions': row[2],
            'exact_matches': row[3],
            'within_one_grade': row[4],
            'avg_deviation': row[5],
    ...
    def get_training_recommendations(self) -> List[str]:
        """Get recommendations for improving AI accuracy"""
        report = self.get_accuracy_report()
        recommendations = []

        if report.get('total_predictions', 0) < 50:
            recommendations.append("Need more training data - submit at least 50 cards to PSA/BGS")

        if report.get('accuracy_percentage', 0) < 80:
            recommendations.append("Accuracy below 80% - review failed predictions for patterns")

        if report.get('avg_deviation', 0) > 1.5:
            recommendations.append("High deviation - consider recalibrating scoring weights")

        if not recommendations:
            recommendations.append("Model performing well - continue normal operations")
