...
class BoxConsolidator:
    ...
    def execute_consolidation(self, source_box_id: str, target_box_id: str) -> Dict:
        ...
        # Update each card's call number in library_data
        source_cards = self.library.box_inventory[source_box_id].copy()
        target_current = len(self.library.box_inventory[target_box_id])
        
        # Update call numbers on the card objects and library_data
        for i, card in enumerate(source_cards):
            old_call = card.get('call_number')
            new_position = target_current + i + 1
            new_call = f"{target_box_id}-{new_position:04d}"
            
            # Update card object
            card['call_number'] = new_call
            card['box_id'] = target_box_id
            card['position'] = new_position
            card['consolidated_from'] = source_box_id
            card['consolidated_date'] = datetime.now().isoformat()
            
            # Update library_data
            if old_call in self.library.library_data:
                card_data = self.library.library_data.pop(old_call)
                card_data['call_number'] = new_call
                card_data['box_id'] = target_box_id
                card_data['position'] = new_position
                self.library.library_data[new_call] = card_data
            
            # Update card_locations index
            card_name = card.get('name')
            if card_name in self.library.card_locations:
                locations = self.library.card_locations[card_name]
                if isinstance(locations, list):
                    if old_call in locations:
                        locations.remove(old_call)
                        locations.append(new_call)
                elif locations == old_call:
                    self.library.card_locations[card_name] = [new_call]
...
if __name__ == "__main__":
    ...
    print()
    print("Integration:")
    print("  from box_consolidation_system import BoxConsolidator")
