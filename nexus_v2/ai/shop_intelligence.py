...
class ShopIntelligenceAI:
    ...
    def __init__(self, shop_name="Default Shop", db_path="E:/MTTGG/AI_LEARNING/shop_intelligence.db"):
        ...
        logger.info(f"Shop Intelligence AI initialized for: {shop_name}")
    ...
    def init_shop_database(self):
        ...
        conn.commit()
        conn.close()
        logger.info("Shop Intelligence Database initialized")
    ...
    def load_neural_networks(self):
        ...
        if self.weights_path.exists():
            ...
            logger.info("Loaded shop AI neural weights")
        else:
            self.neural_weights = {
            ...
            logger.info("Initialized new shop AI networks")
    ...
    def learn_from_scan(self, card_name: str, set_name: str, condition: str, 
        ...
        logger.info(f"Learned from scan: {card_name} ({quantity}x @ ${purchase_price:.2f})")
    ...
    def learn_from_sale(self, card_name: str, sell_price: float, 
        ...
        logger.info(f"AI learned from sale: {card_name} @ ${sell_price:.2f}")
    ...
    def get_inventory_insights(self) -> SimpleNamespace:
        """Get AI-powered inventory insights as SimpleNamespace for attribute access"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        # Return as SimpleNamespace with objects for attribute access
        return SimpleNamespace(
            hot_sellers=[
                SimpleNamespace(
                    card_name=h[0],
                    velocity_score=h[1] or 0,
                    demand_score=h[2] or 0,
                    quantity_on_hand=h[3] or 0,
                    profit_margin=h[4] or 0
                ) for h in hot_sellers
            ],
            dead_stock=[
                SimpleNamespace(
                    card_name=d[0],
                    quantity_on_hand=d[1] or 0,
                    avg_purchase_price=d[2] or 0,
                    times_sold=d[3] or 0,
                    velocity_score=d[4] or 0
                ) for d in dead_stock
            ],
            restock_needed=[
                SimpleNamespace(
                    card_name=r[0],
                    quantity_on_hand=r[1] or 0,
                    optimal_stock_level=r[2] or 0,
                    demand_score=r[3] or 0
                ) for r in restock_needed
            ],
            profit_leaders=[
                SimpleNamespace(
                    card_name=p[0],
                    profit_margin=p[1] or 0,
                    times_sold=p[2] or 0,
                    avg_sell_price=p[3] or 0,
                    avg_purchase_price=p[4] or 0
                ) for p in profit_leaders
            ]
        )
    ...
    def get_profitability_report(self) -> Dict:
        ...
        # Get customer counts
        try:
            cursor.execute('SELECT COUNT(*) FROM customer_intelligence')
            customer_count = cursor.fetchone()[0] or 0
        except:
            customer_count = 0

        try:
            cursor.execute('SELECT COUNT(*) FROM customer_intelligence WHERE vip_status = 1')
    ...
    def print_shop_insights(self):
        ...
        logger.info("=" * 70)
        logger.info(f"SHOP INTELLIGENCE: {self.shop_name}")
        logger.info("=" * 70)

        report = self.get_profitability_report()

        logger.info("FINANCIAL OVERVIEW:")
        logger.info(f"   Inventory Value:     ${report['inventory_value']:,.2f}")
        logger.info(f"   Total Revenue:       ${report['total_revenue']:,.2f}")
        logger.info(f"   Total Profit:        ${report['profit']:,.2f}")
        logger.info(f"   Profit Margin:       {report['profit_margin']:.1f}%")

        logger.info("INVENTORY:")
        logger.info(f"   Unique Cards:        {report['unique_cards']:,}")
        logger.info(f"   Total Quantity:      {report['total_cards']:,}")
        logger.info(f"   Total Sales:         {report['total_sales']:,}")

        insights = self.get_inventory_insights()

        if insights.hot_sellers:
            logger.info("HOT SELLERS (Top 5):")
            for i, card in enumerate(insights.hot_sellers[:5], 1):
                margin = card.profit_margin * 100 if card.profit_margin else 0
                logger.info(f"   {i}. {card.card_name} - Velocity: {card.velocity_score:.2f}, Margin: {margin:.0f}%")

        if insights.restock_needed:
            logger.info("RESTOCK NEEDED:")
            for card in insights.restock_needed[:3]:
                logger.info(f"   - {card.card_name} - {card.quantity_on_hand}/{card.optimal_stock_level} stock")
    ...
    def save_neural_weights(self):
        ...
        logger.info("Shop AI weights saved")
...
if __name__ == "__main__":
    ...
    logger.info("Testing Shop Intelligence AI")
    ...
    # Get insights
    logger.info("INVENTORY INSIGHTS:")
    insights = ai.get_inventory_insights()
    logger.info(json.dumps(insights, indent=2))

    # Get deck recommendations
    logger.info("DECK BUILDING RECOMMENDATIONS:")
    deck_recs = ai.get_deck_building_recommendations("Commander")
    logger.info(json.dumps(deck_recs, indent=2))

    logger.info("Shop Intelligence AI test complete!")
